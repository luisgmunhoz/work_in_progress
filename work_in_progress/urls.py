...
@api.post("/login", auth=None, tags=["login"])
def login(request, data: LoginSchema):
    secret = authenticate(request, username=data.username, password=data.password)
    if secret is not None:
        return 200, {"message": "Logged in succesfully", "x-api-key": secret}
    else:
        raise Exception(403, "Invalid credentials")
